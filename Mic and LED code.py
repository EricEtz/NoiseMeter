...
#time setup can go here (RTC Init) ------------------------------------------------------
...
mainbar = displayio.Group() # The Main Display Group ----------------figure this out
...
#this bit of code below runs through each bar, with a half second pause between each coloring
...
#    for i in range(num_panels+1):   #for i in range of number of panels (defined as 10 panels, for 10 bars)
#        dbLevel(i)                  #run through all values of i, plugging into dbLevel code above
#        time.sleep(0.8)             #wait half a second
#
#    time.sleep(5)                   #once that loop is done, wait 3 seconds
...
while True:                         #this is the operation loop of the device. Refresh screen, keep looping.
    ...
    n = int((mic_pin.value / 65536) * 1000)  # 10-bit ADC format of noise signal            #THIS IS THE LINE TO EDIT
    n = abs(n - 512 - dc_offset)  # Center on zero

    if n >= noise:  # Remove noise/hum from noise signal
        n = n - noise

    # "Dampened" reading (else looks twitchy) - divide by 8 (2^3)                           #OR MAYBE TS THIS ONE ///////////////////////////////////////////////
    lvl = int(((lvl * 7.6) + n) / 8)
